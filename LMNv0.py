...
def act_pred_probabilistic_by_lag(predict_from, result):
    """
    
    I ABANDONED THIS FUNCTION FOR NOW. I MAY RETURN TO IT.
    
    act_pred_probabilistic compares actual to predicted results, demonstrating
    the a number of predictions by showing the P10 and P90 results as error bars.
    Generates a separate plot for each lag.
    ...
    probabilistic_result = result_probs_by_lag(result)
    # remember that the dims are (player, lag, P10P50P90)
    
    # create error array that contains the y error that we're representing.
    # First row is error below, second is error above
    
    err_arr = np.zeros((probabilistic_result.shape[0], probabilistic_result.shape[1], 2))
    for lag in range(probabilistic_result.shape[1]):
        err_arr[:,lag,0] = probabilistic_result[:,lag,1]-probabilistic_result[:,lag,0]
        err_arr[:,lag,1] = probabilistic_result[:,lag,2]-probabilistic_result[:,lag,1]
    err_arr1 = np.transpose(err_arr[:,0,:])
    err_arr2 = np.transpose(err_arr[:,1,:])
    err_arr3 = np.transpose(err_arr[:,2,:])
...
def act_pred_probabilistic_final(predict_from, result_reduced):
    """
    act_pred_probabilistic compares actual to predicted results, demonstrating
    the a number of predictions by showing the P10 and P90 results as error bars.
    ...
    probabilistic_result = result_reduced_probs(result_reduced)
    ...
    error = np.sqrt(mean_squared_error(result_reduced[:,1],predict_from[:,0,-1]))
...
def result_reduced_probs(result_reduced):
    """
    result_probs takes a reduced array of prediction interation results and determines
    the P10 P50 and P90 preditions for each player.
    
    result: dimension 1 is the player; dimension 2 is the iteration
                    
    """
    
    out = np.zeros((result_reduced.shape[0],3))

    for player in range(result_reduced.shape[0]):
        out[player] = np.percentile(result_reduced[player,:],[10,50,90])
    
    return out


#def result_probs_by_lag(result):
#    """
#    result_probs takes an array of prediction interation results and determines
#    the P10 P50 and P90 preditions for each player by lag.
#    
#    result: recall that dimension 1 is the player; dimension 2 is the lag; dimensiion
#                    3 is the iteration
#                    
#    returns nans where nans were present
#                    
#    """
#    
#    # I need to mask the predicitons for nan rows...
#    
#    out = np.zeros((result.shape[0], result.shape[1], 3))
#
#    for player in range(result.shape[0]):
#        for lag in range(result.shape[1]):
#            if np.ma.masked_greater(result[player,lag,:],2).mask.all(): # if there are values to use...
#                out[player,lag] = np.percentile(result[player,lag,:][np.ma.masked_greater(result[player,lag,:],2).mask],[10,50,90])
#            else:
#                out[player,lag] = np.ones((3)) * np.nan
#    return out
#
#
#
...
def result_lag_reduce(result):
    """
    result_lag_reduce takes advantage of the fact that the best results are
    found in the last lag. Unfortunately, this one also contains some nans.
    Let's replace those nans with values from other lags.
    """
    
    for lag in range(1,result.shape[1]): # goes 1,2
        for player in range(result.shape[0]):
            for pred in range(result.shape[2]):
                if result[player,lag,pred] < 2 or not np.isfinite(result[player,lag,pred]):
                    #print('altering result')
                    result[player,lag,pred] = result[player,lag-1,pred]
                if  result[player,lag,pred] > 1000:
                    result[player,lag,pred] = result[player,lag-1,pred]
